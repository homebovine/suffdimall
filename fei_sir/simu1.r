...
simu2 <- function(n, p, beta, std, std1,  k, nonconst, linear){
    ...
    for(i in 1:n){
        if(nonconst == 1){
            lowersigma <- xb[i] ^2 * (diag(1, p -1, p-1) - betaest %*% invbb %*% t(betaest))* std^2
            if(p > 2){
             temp <- svd(lowersigma)
             D <- temp$u %*% (diag(temp$d)^(1/2)) %*% t(temp$v)
         }else{
...
if(linear == 0){       
        x[i, 2:p] <- betaest * as.numeric(invbb * xb[i]^2) +  D %*% (v^(-1/2) * (u[2:p] - m))
...
seff3 <- function(betaest, x, y, std, std1,  nonconst){
    beta <- c(fix, betaest)
    b <- sqrt(3 * t(beta) %*% beta /pi^2) * std1 ^2
    ...
    for(i in 1: n){
        ...
        er = x2 - mx
        if(nonconst == 1){
        csigm =  xb ^2 *  (diag(1, p - 1, p - 1) - betaest %*%invbb %*% t(betaest))
         sigm <- std ^2 * csigm
         invcsigm = ginv(csigm)
          invsigm <- std^(-2) * invcsigm
        dsigma <- 2 * xb * (diag(1, p - 1, p - 1) - betaest %*%invbb %*% t(betaest))
    }else{
...
seff41 <- function(betaest, x, y,std, std1){
...
seff42 <- function(betaest, x, y,std, std1, k){
   ...
   xb <- 1
     mm3 <- fmm(1, k)
    secm <-  mm3[1]#1#
    thirdm <- mm3[2]#1.14#mm3[2]
    fourm <- mm3[3]#12/5 + 3#mm3[3]
    v1em <- vector('list')
...
estimation <- function(beta0, betaini, n, p, itr){
    ...
    simdata <- simu2(n,  p, beta0, std, std1, k, 0, 1)
    y <- simdata[, 1]
    x <- simdata[, 2: (p+ 1)]
   ...
   # temp1 <-try(nlm( seff,estseff,  x, y))# optim(c( estseff), seff, gr = NULL, x, y, method = 'BFGS')#
    estseff <- temp1$par
    temp1 <-optim(c( estseff)* 1.01, seff, gr = NULL, x, y, 1, method = 'BFGS')
    estseff1 <- temp1$par
    temp2 <-optim(c( betaini), seff3, gr = NULL, x, y, std, std1, 0, method = 'Nelder-Mead')
    temp2 <-optim(temp2$par * 1.01, seff3, gr = NULL, x, y, std, std1, 0, method = 'BFGS')
...
res1 <- matrix(NA, 500, (p-1) * 3)
res2 <- matrix(NA, 500, (p-1) * 3)
res3 <- matrix(NA, 500, (p-1) * 3)
#res <- rbind(res, res1)
fix <-1
k <- 1/1000
 for(itr in 1:nsim){
...
pdf('temp.pdf')
boxplot(sqereff, sqereff3, sqereff4, range = 1.5, outline = F, names = c("a", "b", "c"))
dev.off()



evalmc<- function(betaest, std){
      p <- length(betaest) + 1
    beta <- c(fix, betaest)
    b <- sqrt(3 * t(beta) %*% beta /pi^2) * std ^2
    rate = (t(beta) %*% beta)^(-1/2)
...
p <- 3
betaest <- matrix(rnorm(nsim * (p-1)), nsim, p-1)
mk4 <- matrix(NA, nsim, (p-1)^3)
meign <- rep(NA, nsim)
for(itr in 1: nsim){
    temp <- evalmc(betaest[itr, ], 1) 
    vtemp <- svd(temp)
    meign[itr] <- max(vtemp$d)
 mk4[itr, ]<-   as.vector(temp)
} 
res <- res1
m <- round(matrix(abs(apply(res, 2, median) - rep(beta0[2:p], 3)), nrow = 3), 4)
s <- round(matrix(apply(res, 2, mad), nrow = 3), 4)
paste(m[, 1], ' (', s[, 1], ') ', '&', m[, 2], ' (', s[, 2], ') ', '&', m[, 3], ' (', s[, 3], ') ', '\\', sep = '')
