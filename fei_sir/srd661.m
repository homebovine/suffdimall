function [rcorr, pdist, xcorr, init, mave, sr, best] = srd661
times = 100;
n = 300; p = 6;
d = 2;
rcorr = zeros(times,4);
...
datasrd2 = load('datasrd.tab');
datasrd1= load('datasrd7.tab');
datasrd = [datasrd1, datasrd2];
beta = load('beta.tab')
  hessian = zeros(p^2, (p -d) * 2, times);
grad = zeros(p^2, p^2, times);
for ii = 1:times
    ii
    [ init(:,:,ii),best(:,:,ii), grad(:, :, ii),  varbest(:, :, ii)] = SAVEMIM(n,p, ii, datasrd, beta, beta, 0);
...
%			sr(:,:,ii),best(:,:,ii), grad(:, :, ii),  varbest(:, :, ii)] = SAVEMIM(n,p, ii, datasrd, beta, best1, 1);
%    rcorr(ii,:), pdist(ii,:),xcorr(ii,:)
%end
save Example6_Result_661_300_4.mat varbest best times n init

      function [ init, best,grad,  varbest] = SAVEMIM(n,p, ii, datasrd, beta, best,  flg)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                         %
...
for ii = 1:n
    blam11(ii,:) = ((eye(p) - blam(:,:,ii))*(z(ii,:) - invfest(ii,:))')';
    temp = eye(p) - blam(:,:,ii);
    blam22(ii, :) = temp(:) ;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tic
lb = [ -Inf, 0.3, 0.3, -Inf;  0.3, -Inf, -Inf, -Inf];
up = [ Inf, 0.45, 0.45, Inf; 0.45, Inf, Inf, Inf];
invS = eye(p^2);
size(invS)
flg
if (flg == 1)
grad = gobjcon(best, blam11, blam22, z, p, zz);
invS = inv(grad);
end
...
[best,ee,eflag, output, lambda, grad, hessian1] = fmincon(@(best) objcon(best,blam11,blam22,z,p,  zz, beta,invS),beta(3:6, :),[],[],[],[],[],[],[],optimset('LargeScale','on', 'TolCon',1e-8,'TolFun',1e-8,'algorithm','active-set', 'MaxIter', 10000, 'MaxFunEvals', 10000));
best = [beta(1:2, 1:2); best];
grad = gobjcon(best, blam11, blam22, z, p, zz);
end

		    %[best,ee,eflag, output] = fminsearch(@(best) objcon1(best,blam11,blam22,z,p,zz, beta, invS),beta(3:6, :),optimset( 'TolX',1e-6,'TolFun',1e-6, 'MaxIter', 10000, 'MaxFunEvals', 10000));
toc
%		    size(best)
beta2 = [beta(3:6, 1); beta(3:6, 2)];
		   % hobjcon(beta2,blam11,blam22,z,p, dim, zz, beta)
		    %[best,resnorm,residual,exitflag,output,lambda,hessian]    = lsqnonlin(@(best) %hobjcon1(best,blam11,blam22,z,p, dim,  zz, beta, eye(p^2)), beta2, [], [], optimset('LargeScale','%on',  'TolX', 1e-6,  'TolFun',1e-4,'Algorithm','levenberg-marquardt', 'MaxIter', 1000, 'MaxFunEval%s', 10000)); 		    best = reshape(best, p-2, 2);
output
...
best2 = [best(3:6, 1); best(3:6, 2)]
		   ...
		   hessian = numjacobian(best2, blam11, blam22, z, p, dim, zz, beta);
	%	    invhess = inv(hessian1);
	%	    varbest = invhess * hessian'* invS * grad * invS' *  hessian * invhess'
%invS = inv(grad);
		    invgrad = (hessian' * invS * hessian)^(-1) * hessian'* invS;

			       varbest = invgrad * (grad)  * invgrad';%inv(hessian' * inv(grad) * hessian)% ;%;%;%;%%
...
zres = z - zfest;
%mblam11 = kernelregression(z*best,blam11,z*best,'kernel','epanechinikov');
%blam11 = blam11 - mblam11;
mblam22 = kernelregression(z*best,blam22,z*best,'kernel','epanechinikov');
blam22 = blam22 - mblam22;
for(i = 1 : n)
mblam22temp(:, :, i) = reshape(blam22(i, :), p, p);
end
blam22 = mblam22temp;
