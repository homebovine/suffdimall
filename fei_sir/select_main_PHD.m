function [LICa,LICb,ee,eflag,dis,betahat,div, div1]=select_main_PHD(x,y,dim,beta,ti, pini, betapre)
    %global W0
    warning off;
    [n,p] = size(x);
    z  = x;
...
betainit = betapre(ii + 1:p, iiinit:iiend )
	if pini ~= 1
        [best,ee(1,ii),eflag(1,ii)] = fminsearch(@(best)objcon(best,z,y,zz,n),betainit,options);
	[best,ee(1,ii),eflag(1,ii),output,grad,hessian] = fminunc(@(best)objcon(best,z,y,zz,n),best* 1.01,options);
	end
	
	if pini == 1 
        [best,ee(1,ii),eflag(1,ii),output,grad,hessian] = fminunc(@(best)objcon(best,z,y,zz,n),betainit,options);
	end
        if ii==ti 
            [best betainit]
	    ...
	    zBtemp = zeros((p - ii)*ii, 1 );
	      ...
	      Btemp = reshape(Btemp, p-ii, ii);
                b  = bfunction(best-Btemp,z,y,zz,n);
                b2 = bfunction(best+Btemp,z,y,zz,n);
...
%             hessian_inv = inv(hessian/n);
        ...
        b1        = bfunction(gamma1,z,y,zz,n);
        b2        = bfunction(gamma2,z,y,zz,n);     
        [W1a,W1b] = Wb(b1,(p-ii-1)*(ii+1));  
        [W2a,W2b] = Wb(b2,(p-ii-1)*(ii+1)); 
        L1a        = objconV(gamma1,z,y,zz,n,W1a);
        L1b        = objconV(gamma1,z,y,zz,n,W1b);
        L2a        = objconV(gamma2,z,y,zz,n,W2a);
        L2b        = objconV(gamma2,z,y,zz,n,W2b);
        LICa(1,ii)    = L1a+L2a;
        LICb(1,ii)    = L1b+L2b;
...
function ee = objcon(best,z,y,zz,n)
b = bfunction(best,z,y,zz,n);
ee = mean(b,2)'*mean(b,2)*n;%'

function b =bfunction(best,z,y,zz,n)
best = [eye(length(best(1,:))); best];
zfest = kernelregression(z*best,zz,z*best,'kernel','epanechinikov');
zzres = zz - zfest;
fest = kernelregression(z*best,y,z*best, ...
...
function ee = objconV(best,z,y,zz,n,W)
b = bfunction(best,z,y,zz,n);
