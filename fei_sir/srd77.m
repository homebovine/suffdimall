...
eflag = []; 
beta = [1,0,6.560136e-01,5.981006e-01,6.606420e-01,5.438697e-01;0,1,4.934525e-02,1.505425e-01,7.792817e-01,4.617166e-01]'

%datasrd2 = load('datasrd.tab');
%datasrd1 = load('datasrd4.tab');
%datasrd = [datasrd1, datasrd2];

%load('inibeta2dim.mat');
%beta = init;
%beta = best
 %beta = beta * diag([1/sqrt(6), -1/sqrt(6)]);
for ii = 1:times
    ii
    [ init(:,:,ii), best(:,:,ii), grad(:, :, ii), varbest(:, :, ii), eflag(ii), ee] = DRMIM(n,p, ii, datasrd, beta, grad(:, :, 1), 0);
...
save Example7_real.mat best varbest n times init ee

      function [init, best, grad, varbest, eflag, ee] = DRMIM(n,p, ii, datasrd, beta, grad, flag)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                         %
...
%                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
best = ss^(-1/2) * best * inv(beta(1:2, 1:2))
init = ss^(-1/2)*init; 
