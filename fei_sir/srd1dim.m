...
init = zeros(p,1,times);
mave = zeros(p,1,times);
sr = zeros(p,1,times);
best = zeros(p,1,times);
varbest = zeros((p -2), (p -2),times);
...
for ii = 1:times
    ii
    [ init,best, grad,  varbest] = SAVEMIM(n,p, ii, datasrd, beta, grad(:, :, 1), 0);
    rcorr(ii,:), pdist(ii,:),xcorr(ii,:)
end
%wgrad = mean(grad(:, :, 1:10), 3);
%%best1 = median(best, 3);
%for ii = 1:times
%    ii
%    [rcorr(ii,:), pdist(ii,:), ...
%        xcorr(ii,:), init(:,:,ii),mave(:,:,ii),...
%			sr(:,:,ii),best(:,:,ii), grad(:, :, ii),  varbest(:, :, ii)] = SAVEMIM(n,p, ii, datasrd, beta, wgrad, 1);
%    rcorr(ii,:), pdist(ii,:),xcorr(ii,:)
%end
save Example6_real.mat varbest best times n init

      function [init, best,grad,  varbest] = SAVEMIM(n,p, ii, datasrd, beta, grad,  flg)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                         %
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tic
lb = [ -Inf, -Inf, -Inf, -1;  -Inf, -Inf, -Inf, -1];
up = [ Inf, Inf, Inf, 1; Inf, Inf, Inf, 1];
invS = eye(p^2);
