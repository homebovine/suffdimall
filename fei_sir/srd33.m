...
varbest = zeros(4 * 2,4* 2,times);
grad = zeros(p^2, p^2, times);
%datasrd2 = load('datasrd_norm1.tab');
%2Bdatasrd1 = load('datasrd4_norm1.tab');
...
for ii = 1:times
    ii
    [ init(:,:,ii), best(:,:,ii), grad(:, :, ii), varbest(:, :, ii)] = PHDMIM(n,p, ii, datasrd, beta, beta, 0);
...
save Example3_real.mat best varbest times n init

      function [ init,  best, grad, varbest] = PHDMIM(n,p, ii, datasrd, beta, best, flag)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                         %
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tic
%grad = gobjcon(beta, z, y, zz, n);
%[U, S, V] = svd(grad);
%Si = S;
%Si(S<=0) = 1e-7;
%Si = S.^(-1);
invS = eye(p^2);%pinv(grad);% U * Si * V';%inv(grad + eye(p^2) * 1e-6);
if(flag == 1)
grad = gobjcon(best, z, y, zz, n);
invS = pinv(grad);
end
for i = 1
if i ~= 1
...
[best,ee,eflag, output,lambda, grad, hessian] = fmincon(@(best) objcon(best,z,y,zz,n, beta, invS),beta(3:6, :),...
...
%invS = pinv(grad);
hessian = numjacobian( best2, z, y, zz, p, dim, n, beta);
...
%best, z,y,zz,n
% tic
% [best,ee,eflag, output,lambda, grad, hessian] = fmincon(@(best) gobjcon(best,z,y,zz,n),beta,...
%     [],[],[],[],-1,1,@(best) gconstraint(best, resbest),...
%     optimset('LargeScale','on','TolCon',1e-4,'TolFun',1e-4,'algorithm','active-set'));
% toc
% fval = jacobian(gbobjcon(best(:, 1), beta(:, 2) z, y, zz, n), best1)
 %invgrad = pinv(hessian);
 varbest =  invgrad * (grad)  * invgrad';
