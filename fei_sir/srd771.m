function [rcorr, pdist, xcorr, init, mave, sr, best] = srd771
times = 100;
n = 300; p = 6;
d = 2;
...
beta = load('beta.tab')
 %beta = beta * diag([1/sqrt(6), -1/sqrt(6)]);
for ii = 1:times
    ii
    [ init(:,:,ii),best(:,:,ii), grad(:, :, ii), varbest(:, :, ii), eflag(ii)] = DRMIM(n,p, ii, datasrd, beta, beta, 0);
...
%			sr(:,:,ii),best(:,:,ii), grad(:, :, ii), varbest(:, :, ii), eflag(ii)] = DRMIM(n,p, ii, datasrd, beta, best1, 1);
%    rcorr(ii,:), pdist(ii,:),xcorr(ii,:)
%end
%
save Example7_Result_norm771_300_7.mat varbest best times n init

      function [ init, best, grad, varbest, eflag] = DRMIM(n,p, ii, datasrd, beta, best, flag)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                         %
...
lb(:) = -1;
up(:) = 1;
%grad = gobjcon(beta, blam11, blam22, invsqfest, invfest, z, p);
invS = eye(p^2);
if (flag == 1)
grad = gobjcon(best, blam11, blam22, invsqfest, invfest, z, p);
invS = inv(grad);
end
[best,ee,eflag, output,lambda, grad, hessian] = fmincon(@(best) objcon(best,blam11,blam22,...
    invsqfest,invfest,z,p,  beta, invS),beta(3:6, :),[],[],[],[],[],[],[],optimset('LargeScale','on', 'TolCon',1e-8,'TolFun',1e-8,'algorithm','active-set', 'MaxIter', 10000, 'MaxFunEvals', 10000));
toc
...
grad = gobjcon(best, blam11, blam22, invsqfest, invfest, z, p);
%invS = inv(grad);
 hessian = numjacobian(best2, blam11, blam22, invsqfest, invfest, z, p, dim, beta);
...
% The objective function for fmincon
	 ...
	 best = [beta(1:2, 1:2); best];
n = size(z,1);
invfestm = kernelregression(z*best,invfest,z*best, 'kernel', 'epanechinikov');
invfest = invfest - invfestm;
...
blam55temp = kernelregression(z*best,Iminus,z*best, 'kernel','epanechinikov');
for ii = 1:n
    temp = reshape(blam55temp(ii, :), p, p);
    blam55(:,:,ii) = (eye(p)-invsqfest(:,:,ii) - temp )*blam44(:,:,ii);
end
...
blam55temp1 = kernelregression(z*best,Iminus,z*best, 'kernel','epanechinikov');

mblam55 = zeros(n, p * p);
for ii = 1:n
    temp = reshape(blam55temp1(ii, :), p, p);
	   blam55temp = - (eye(p)-invsqfest(:,:,ii) - temp)*blam44(:,:,ii) + blam22 * invfest(ii, :)' * zres(ii, :) + trace(blam22) * invfest(ii, :)' * zres(ii, :);
